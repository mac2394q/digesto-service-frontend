...
import {
    DigestoService
} from 'src/app/Modules/Digesto/Service/digesto';
import {
    HTMLInput
...
export class DocumentosComponent implements AfterViewInit {
    ...
    async loadObservacionesoDocumentos(codigo) {
        const dTable: any[] = [];
        const data3 = await this.apiRest.getObservacionesDocumento(codigo).toPromise();

        const dataBody3 = data3["jsonFormatObject"]["body"];
        console.log(dataBody3);

        for (const ele of dataBody3) {
            dTable.push({

                OBSERVACION: new HTMLInput({
                    alias: 'Observacion',
                    value: ele["OBSERVACION"]
                }),

                USUARIO_REGISTRO: new HTMLInput({
                    alias: 'Usuario ',
                    value: ele["USUARIO_REGISTRO"]
                }),

                FECHA_REGISTRO: new HTMLInput({
                    alias: 'Fecha de registro',
                    value: ele["FECHA_REGISTRO"]
                }),
    ...
    async formularioCreacionObservacion() {
        ...
        const dTable: any[] = [];
            ...
            dTable.push({
                ...
                usuario: new HTMLInput({
                    type: 'text',
                    alias: 'Codigo de documento',
                    columnClass: "col-3",
                    required: true
                }),
                crearObservacion: new HTMLInput({
                    type: 'button',
                    columnClass: 'col-2',
                    alias: 'Crear Observacion',
    ...
    async loadDetailInfoDocumentos(codigo) {
        ...
        const data2 = await this.apiRest.getVersionesDocumento(codigo).toPromise();

        const dataBody2 = data2["jsonFormatObject"]["body"];
        ...
        return {
            ...
            observaciones: new HTMLInput({
                type: 'DTMixture',
                columnClass: 'col-2',
                alias: 'observaciones del documento',
                modalService: () => this.loadObservacionesoDocumentos(codigo) ,
                modalValue: 'observacionesList'
            }),

            title3: new HTMLInput({
                type: 'title',
                value: 'Nueva version del documento',
                titleUnHide: true
            }),

            nuevaVersion: new HTMLInput({
                type: 'file',
                alias: 'Ruta almacenamiento',
                columnClass: "col-6",
                required: true
            }),
            crearVersion: new HTMLInput({
                type: 'button',
                columnClass: 'col-3',
                alias: 'Nueva version del documento',

            }),
            title4: new HTMLInput({
                type: 'title',
                value: 'Nueva observacion',
                titleUnHide: true
            }),
            ...
            title2: new HTMLInput({
                type: 'title',
                value: 'Versiones del documento',
                titleUnHide: true
            }),
            list: Array.from(dataBody2, (data2: any) => {
                return {
                    'nombre': new HTMLInput({
                        type: 'text',
                        alias: 'Nombre del documento',
                        value: data2["nombre"],
                        columnClass: 'col-3',
                    ...
                    'tipo': new HTMLInput({
                        type: 'text',
                        alias: 'tipo del documento',
                        value: data2["tipo"],
                        columnClass: 'col-3',
                        inputClass: "form-control"
                    }),
                    'tamano': new HTMLInput({
                        type: 'text',
                        alias: 'tamaño del documento',
                        value: data2["tamano"],
                        columnClass: 'col-2',
                        inputClass: "form-control",
                        readonly: true
